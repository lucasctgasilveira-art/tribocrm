...
const CSS = `
  ...
  .im-body::-webkit-scrollbar-thumb{background:#22283a;border-radius:4px}.im-body{scrollbar-width:thin;scrollbar-color:#22283a transparent}
`
...
export default function ImportLeadsModal({ open, onClose, onImported }: Props) {
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 520, maxWidth: '90vw', maxHeight: '90vh', background: '#161a22', border: '1px solid #22283a', borderRadius: 16, zIndex: 51, display: 'flex', flexDirection: 'column', animation: 'imScaleIn 0.2s ease-out' }}>
        {/* Header */}
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #22283a', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexShrink: 0 }}>
          <div>
            <h2 style={{ fontSize: 17, fontWeight: 700, color: '#e8eaf0', margin: 0 }}>Importar Leads</h2>
            <p style={{ fontSize: 13, color: '#6b7280', marginTop: 2 }}>Importe sua base de leads via planilha Excel</p>
          </div>
          <button onClick={handleClose} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#6b7280', padding: 4 }}><X size={18} strokeWidth={1.5} /></button>
        </div>

        {/* Step indicator */}
        <div style={{ padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0, borderBottom: '1px solid #22283a', flexShrink: 0 }}>
          {steps.map((s, i) => {
              ...
              <div key={s.n} style={{ display: 'flex', alignItems: 'center' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                  <div style={{
                    ...
                    background: done || isCurrent ? '#f97316' : '#22283a',
                    color: done || isCurrent ? '#fff' : '#6b7280',
                  }}>{done ? '✓' : s.n}</div>
                  <span style={{ fontSize: 10, color: active || isCurrent ? '#f97316' : '#6b7280', whiteSpace: 'nowrap' }}>{s.label}</span>
                </div>
                {i < steps.length - 1 && (
                  <div style={{ width: 48, height: 2, background: done || (step === 3 && i === 1) ? '#f97316' : '#22283a', margin: '0 8px', marginBottom: 16 }} />
                )}
              </div>
            )
          })}
        ...
        <div className="im-body" style={{ padding: 24, overflowY: 'auto', flex: 1 }}>
          {step === 1 && (
            <>
              {/* Download template */}
              <div style={{ background: '#0f1117', border: '1px solid #22283a', borderRadius: 12, padding: 24, textAlign: 'center', marginBottom: 20 }}>
                <Download size={32} color="#f97316" strokeWidth={1.5} />
                <h3 style={{ fontSize: 15, fontWeight: 600, color: '#e8eaf0', margin: '12px 0 6px' }}>Baixe o modelo Excel</h3>
              ...
              {/* Columns reference */}
              <div style={{ borderTop: '1px solid #22283a', paddingTop: 16, marginBottom: 16 }}>
                <span style={{ fontSize: 11, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600 }}>Colunas do modelo</span>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
                  {columns.map(c => (
                    <span key={c.name} style={{ background: '#22283a', color: '#9ca3af', borderRadius: 4, padding: '3px 8px', fontSize: 11 }}>
          ...
          {step === 3 && uploadState === 'idle' && (
            <div
              ...
              style={{
                border: `2px dashed ${dragOver ? '#f97316' : '#22283a'}`,
                borderRadius: 12, padding: 40, textAlign: 'center', cursor: 'pointer',
                background: dragOver ? 'rgba(249,115,22,0.04)' : 'transparent',
                transition: 'all 0.2s',
              }}
            >
              <Upload size={32} color="#6b7280" strokeWidth={1.5} />
              <div style={{ fontSize: 14, color: '#e8eaf0', marginTop: 12 }}>Arraste o arquivo aqui</div>
              <div style={{ fontSize: 13, color: '#6b7280', marginTop: 4 }}>ou clique para selecionar</div>
              <div style={{ fontSize: 11, color: '#6b7280', marginTop: 8 }}>.xlsx, .xls, .csv</div>
          ...
          {step === 3 && (uploadState === 'uploaded' || uploadState === 'importing') && (
            <>
              {/* File info */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#0f1117', border: '1px solid #22283a', borderRadius: 8, padding: 12, marginBottom: 16 }}>
                <FileSpreadsheet size={24} color="#22c55e" strokeWidth={1.5} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 500, color: '#e8eaf0' }}>{fileName}</div>
                  <div style={{ fontSize: 11, color: '#6b7280' }}>{fileSize}</div>
                </div>
                <button onClick={() => { setUploadState('idle'); setFileName(''); setFileSize('') }} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#6b7280', padding: 4 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13, color: '#e8eaf0' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 13, color: '#9ca3af' }}>Etapa inicial:</span>
                  <select value={initialStage} onChange={e => setInitialStage(e.target.value)} style={{
                    background: '#111318', border: '1px solid #22283a', borderRadius: 6, padding: '5px 10px',
                    fontSize: 12, color: '#e8eaf0', outline: 'none', cursor: 'pointer',
                  }}>
          ...
          {uploadState === 'done' && (
            <div style={{ textAlign: 'center', padding: 20 }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 600, color: '#e8eaf0', marginTop: 12 }}>47 leads importados com sucesso!</div>
            </div>
          )}
        </div>

        {/* Footer */}
        <div style={{ padding: '16px 24px', borderTop: '1px solid #22283a', display: 'flex', justifyContent: step === 3 && uploadState === 'uploaded' ? 'space-between' : 'flex-end', flexShrink: 0 }}>
          {step === 3 && (uploadState === 'uploaded' || uploadState === 'importing') && (
            <button onClick={() => { setStep(1); setUploadState('idle') }} disabled={uploadState === 'importing'} style={{
              background: 'transparent', border: '1px solid #22283a', borderRadius: 8,
              padding: '9px 20px', fontSize: 13, color: '#9ca3af', cursor: 'pointer',
              opacity: uploadState === 'importing' ? 0.5 : 1,
            }}>Voltar</button>
          )}
          {step === 1 && (
            <button onClick={handleClose} style={{ background: 'transparent', border: '1px solid #22283a', borderRadius: 8, padding: '9px 20px', fontSize: 13, color: '#9ca3af', cursor: 'pointer' }}>Fechar</button>
          )}
          {step === 3 && uploadState === 'idle' && (
            <button onClick={handleClose} style={{ background: 'transparent', border: '1px solid #22283a', borderRadius: 8, padding: '9px 20px', fontSize: 13, color: '#9ca3af', cursor: 'pointer' }}>Fechar</button>
          )}
