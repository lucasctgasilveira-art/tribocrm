    ...
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div style={card}>
          ...
          <button onClick={() => { const token = localStorage.getItem('accessToken'); window.location.href = `${import.meta.env.VITE_API_URL || 'http://localhost:3002'}/oauth/google/authorize?token=${token}` }} style={{ width: '100%', background: '#f97316', color: '#fff', border: 'none', borderRadius: 8, padding: '9px 0', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Conectar Gmail</button>
        ...
        <div style={card}>
          ...
          <button onClick={() => { const token = localStorage.getItem('accessToken'); window.location.href = `${import.meta.env.VITE_API_URL || 'http://localhost:3002'}/oauth/calendar/authorize?token=${token}` }} style={{ width: '100%', background: '#f97316', color: '#fff', border: 'none', borderRadius: 8, padding: '9px 0', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Conectar Calendar</button>
        </div>
      </div>
