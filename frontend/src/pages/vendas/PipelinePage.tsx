                      ...
                      <div key={lead.id} draggable onDragStart={(e) => onDragStart(e, lead.id)} onDragEnd={onDragEnd} onClick={() => setSelectedLead(lead)}
                        ...
                        <div style={{ marginTop: 10, borderTop: '1px solid var(--border)', paddingTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          ...
                          <div style={{ display: 'flex', gap: 2, marginLeft: 'auto' }}>
                            <ActBtn color="#25d166" onClick={() => { const p = lead.phone.replace(/\D/g, ''); if (p && p !== '') { const full = p.length <= 11 ? `55${p}` : p; window.open(`https://wa.me/${full}#tribocrm-phone=${full}`, '_blank'); api.post(`/leads/${lead.id}/interactions`, { type: 'WHATSAPP', notes: 'Contato via WhatsApp' }).catch(() => {}) } }}><MessageCircle size={14} strokeWidth={1.5} /></ActBtn>
