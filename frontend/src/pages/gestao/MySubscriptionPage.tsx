        ...
        <div style={{ ...card, padding: 24 }}>
          ...
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <PayOpt icon={<QrCode size={18} strokeWidth={1.5} />} color="#22c55e" label="PIX" onClick={() => setPayModal(true)} />
              <PayOpt icon={<FileText size={18} strokeWidth={1.5} />} color="#3b82f6" label="Boleto" onClick={() => setPayModal(true)} />
              <PayOpt icon={<CreditCard size={18} strokeWidth={1.5} />} color="#f97316" label="Cartão" onClick={() => setPayModal(true)} />
            </div>
          </div>
        </div>
      </div>
            ...
            {payments.map(p => {
              ...
              return (
                <tr key={p.id}>
                  ...
                  <td style={tdS}>
                    <button onClick={() => { if (p.status === 'Vencido') setPayModal(true) }} style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 10px', fontSize: 11, color: p.status === 'Vencido' ? '#ef4444' : 'var(--text-secondary)', cursor: 'pointer' }}>
...
export function _PixModal({ onClose }: { onClose: () => void }) {
...
export function _BoletoModal({ onClose }: { onClose: () => void }) {
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')
...
function PaymentFlowModal({ onClose, onSuccess, showPlanSelection }: { onClose: () => void; onSuccess?: () => void; showPlanSelection?: boolean }) {
  ...
  const [method, setMethod] = useState<'PIX' | 'BOLETO' | 'CARTAO' | null>(null)
  const [loading, setLoading] = useState(false)
  const [pixRes, setPixRes] = useState<{ pixCopiaECola: string } | null>(null)
  const [boletoRes, setBoletoRes] = useState<{ boletoUrl: string; barCode: string } | null>(null)
  const [downgrade, setDowngrade] = useState(false)
  const [copied, setCopied] = useState(false)
  const cur = ap.find(p => p.current); const sel = ap.find(p => p.id === planId)
  const isDG = !!(showPlanSelection && sel && cur && sel.monthly < cur.monthly)
  const bm = sel?.monthly ?? cur?.monthly ?? 349; const ym = Math.round(bm * 0.85); const yt = ym * 12; const sv = (bm * 12) - yt
  const dp = cycle === 'Y' ? yt : bm; const done = pixRes || boletoRes || downgrade
  const showS = !showPlanSelection || (planId && !isDG)
  async function go() {
    if (isDG) { setDowngrade(true); return }; if (!method) return; setLoading(true)
    try {
      const d = `TriboCRM ${sel?.name ?? 'Pro'} — ${cycle === 'Y' ? 'Anual' : 'Mensal'}`
      if (method === 'PIX') { const e = showPlanSelection ? '/payments/upgrade' : '/payments/pix'; const b = showPlanSelection ? { newPlanId: planId, paymentMethod: 'PIX' } : { value: dp, description: d }; const r = await api.post(e, b); setPixRes({ pixCopiaECola: r.data.data.pixCopiaECola }) }
  ...
  return (<>
    ...
    <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 500, maxWidth: '90vw', maxHeight: '90vh', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, zIndex: 51, display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between' }}>
        <h2 style={{ fontSize: 17, fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>{showPlanSelection ? 'Mudar de plano' : 'Realizar pagamento'}</h2>
        <button onClick={onClose} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: 4 }}><X size={18} strokeWidth={1.5} /></button>
      </div>
      <div style={{ padding: 24, overflowY: 'auto', flex: 1 }}>
        {pixRes ? <div style={{ textAlign: 'center' }}><div style={{ fontSize: 20, fontWeight: 800, color: '#22c55e', marginBottom: 8 }}>R$ {dp.toFixed(2)}</div><div style={{ background: '#fff', borderRadius: 12, padding: 16, display: 'inline-block', marginBottom: 16 }}><QRCodeSVG value={pixRes.pixCopiaECola} size={180} level="M" includeMargin /></div><div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', fontSize: 11, color: 'var(--text-secondary)', wordBreak: 'break-all', marginBottom: 12, textAlign: 'left' }}>{pixRes.pixCopiaECola}</div><button onClick={() => { navigator.clipboard.writeText(pixRes.pixCopiaECola); setCopied(true); setTimeout(() => setCopied(false), 2000) }} style={{ background: '#22c55e', color: '#fff', border: 'none', borderRadius: 8, padding: '9px 20px', fontSize: 13, fontWeight: 600, cursor: 'pointer', width: '100%' }}><Copy size={14} style={{ verticalAlign: 'middle', marginRight: 6 }} />{copied ? 'Copiado!' : 'Copiar codigo PIX'}</button></div>
        : boletoRes ? <div style={{ textAlign: 'center' }}><div style={{ fontSize: 20, fontWeight: 800, color: '#3b82f6', marginBottom: 16 }}>R$ {dp.toFixed(2)}</div>{boletoRes.barCode && <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 12px', fontSize: 12, color: 'var(--text-secondary)', fontFamily: 'monospace', wordBreak: 'break-all', marginBottom: 12 }}>{boletoRes.barCode}</div>}{boletoRes.boletoUrl && <a href={boletoRes.boletoUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: '#3b82f6', color: '#fff', borderRadius: 8, padding: '9px 0', fontSize: 13, fontWeight: 600, textDecoration: 'none', width: '100%' }}><FileText size={14} /> Abrir boleto</a>}</div>
        : downgrade ? <div style={{ textAlign: 'center' }}><div style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 8, padding: 16, marginBottom: 20, fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6 }}>O downgrade para <strong style={{ color: 'var(--text-primary)' }}>{sel?.name}</strong> sera aplicado no proximo ciclo. Voce continuara com o plano atual ate <strong style={{ color: 'var(--text-primary)' }}>05/05/2026</strong>.</div><button onClick={onClose} style={{ background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 24px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Entendi</button></div>
        : <>
          {showPlanSelection && <div style={{ marginBottom: 20 }}><div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 10, fontWeight: 600 }}>Escolha o plano</div><div style={{ display: 'flex', gap: 10 }}>{ap.map(p => <div key={p.id} onClick={() => !p.current && setPlanId(p.id)} style={{ flex: 1, padding: 14, borderRadius: 10, border: `2px solid ${planId === p.id ? 'var(--accent)' : 'var(--border)'}`, background: planId === p.id ? 'rgba(249,115,22,0.06)' : 'transparent', cursor: p.current ? 'default' : 'pointer', opacity: p.current ? 0.5 : 1, textAlign: 'center' }}><div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>{p.name}</div><div style={{ fontSize: 18, fontWeight: 800, color: planId === p.id ? 'var(--accent)' : 'var(--text-primary)', marginTop: 4 }}>R$ {p.monthly}</div>{p.current && <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>Atual</div>}</div>)}</div>{isDG && <div style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 8, padding: 10, marginTop: 10, fontSize: 12, color: 'var(--text-secondary)' }}>Downgrade sera aplicado no proximo ciclo.</div>}</div>}
          {showS && <>
            <div style={{ marginBottom: 20 }}><div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 10, fontWeight: 600 }}>Escolha o ciclo</div><div style={{ display: 'flex', gap: 10 }}>
      ...
      {!done && method !== 'CARTAO' && <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between' }}>
        <button onClick={onClose} style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, padding: '9px 20px', fontSize: 13, color: 'var(--text-secondary)', cursor: 'pointer' }}>Cancelar</button>
        <button onClick={go} disabled={(!method && !isDG) || loading} style={{ background: (method || isDG) ? 'var(--accent)' : 'var(--border)', border: 'none', borderRadius: 8, padding: '9px 20px', fontSize: 13, fontWeight: 600, color: (method || isDG) ? '#fff' : 'var(--text-muted)', cursor: (method || isDG) ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: 6 }}>{loading && <Loader2 size={14} className="animate-spin" />}{isDG ? 'Confirmar downgrade' : loading ? 'Gerando...' : `Pagar R$ ${dp.toFixed(2)}`}</button>
      </div>}
    </div>
  </>)
}
...
export function _UpgradeModal({ onClose }: { onClose: () => void }) {
  const upgradePlans = [
...
export function _PayNowModal({ onClose }: { onClose: () => void }) {
  const [method, setMethod] = useState<'PIX' | 'BOLETO' | 'CARTAO'>('PIX')
  const [loading, setLoading] = useState(false)
