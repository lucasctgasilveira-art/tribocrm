  ...
  return (
    <AppLayout menuItems={adminMenuItems}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700, color: '#e8eaf0', margin: 0 }}>Equipe Interna</h1>
      ...
      <div style={{ display: 'flex', alignItems: 'center', fontSize: 13, marginBottom: 20 }}>
        <span style={{ color: '#6b7280' }}>Total</span><span style={{ color: '#e8eaf0', fontWeight: 700, marginLeft: 4 }}>4</span>
        <span style={{ color: '#22283a', margin: '0 10px' }}>|</span>
        <span style={{ color: '#6b7280' }}>Super Admins</span><span style={{ color: '#f97316', fontWeight: 700, marginLeft: 4 }}>1</span>
        <span style={{ color: '#22283a', margin: '0 10px' }}>|</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        {members.map(m => (
          <div key={m.id} style={{ background: '#161a22', border: '1px solid #22283a', borderRadius: 12, padding: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: '#e8eaf0' }}>{m.name}</div>
            ...
            <div style={{ fontSize: 11, color: '#6b7280', marginBottom: 14 }}>Último acesso: {m.lastAccess}</div>
            <div style={{ display: 'flex', gap: 6, position: 'relative' }}>
              ...
              <button onClick={() => setOpenMenu(openMenu === m.id ? null : m.id)} style={{ width: 32, height: 32, borderRadius: 8, border: '1px solid #22283a', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#9ca3af' }}>
                <MoreHorizontal size={14} strokeWidth={1.5} />
              </button>
              {openMenu === m.id && (
                <div style={{ position: 'absolute', right: 0, bottom: 40, zIndex: 20, background: '#161a22', border: '1px solid #22283a', borderRadius: 8, boxShadow: '0 8px 24px rgba(0,0,0,0.4)', minWidth: 160, padding: '4px 0' }}>
                  {menuOpts.map(opt => <div key={opt} onClick={() => setOpenMenu(null)} style={{ padding: '8px 14px', fontSize: 13, color: opt === 'Desativar' ? '#ef4444' : '#e8eaf0', cursor: 'pointer' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.04)' }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent' }}>{opt}</div>)}
                </div>
              )}
            </div>
          </div>
