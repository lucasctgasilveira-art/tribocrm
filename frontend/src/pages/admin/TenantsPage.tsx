    ...
    <AppLayout menuItems={adminMenuItems}>
        ...
        {tabFilters.map(tab => (
          <button key={tab.key} onClick={() => { setActiveTab(tab.key); setStatusFilter(''); setPage(1) }}
            style={{
              ...
              border: `1px solid ${activeTab === tab.key ? 'rgba(249,115,22,0.3)' : '#22283a'}`,
      ...
      {loading ? (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 60, gap: 10 }}>
          ...
          <span style={{ fontSize: 14, color: '#6b7280' }}>Carregando clientes...</span>
        </div>
      ) : (
        <div style={{ background: '#161a22', border: '1px solid #22283a', borderRadius: 12, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {tenants.length === 0 ? (
                <tr><td colSpan={7} style={{ padding: 40, textAlign: 'center', color: '#6b7280', fontSize: 14 }}>Nenhum cliente encontrado</td></tr>
              ) : tenants.map(t => {
                ...
                return (
                  <tr key={t.id} onMouseEnter={() => setHoveredRow(t.id)} onMouseLeave={() => setHoveredRow(null)}
                    style={{ borderBottom: '1px solid #22283a', background: hoveredRow === t.id ? 'rgba(255,255,255,0.02)' : 'transparent' }}>
                    <td style={{ padding: '14px 20px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 32, height: 32, borderRadius: 8, background: '#22283a', fontSize: 11, fontWeight: 700, color: '#e8eaf0', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{ini(t.name)}</div>
                        <div>
                    ...
                    <td style={{ padding: '14px 20px', fontSize: 13, color: '#e8eaf0' }}>{t._count.users}</td>
                    <td style={{ padding: '14px 20px', fontSize: 13, color: '#e8eaf0' }}>{t._count.leads.toLocaleString('pt-BR')}</td>
                    <td style={{ padding: '14px 20px', fontSize: 13, fontWeight: 500, color: t.status === 'TRIAL' || t.status === 'CANCELLED' ? '#6b7280' : '#e8eaf0' }}>
                    ...
                    <td style={{ padding: '14px 20px', position: 'relative' }}>
                      <button onClick={() => setOpenMenu(openMenu === t.id ? null : t.id)}
                        style={{ width: 28, height: 28, borderRadius: 6, border: '1px solid #22283a', background: openMenu === t.id ? '#22283a' : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#9ca3af' }}>
                        <MoreHorizontal size={14} strokeWidth={1.5} />
                      </button>
                      {openMenu === t.id && (
                        <div style={{ position: 'absolute', right: 20, top: 48, zIndex: 20, background: '#161a22', border: '1px solid #22283a', borderRadius: 8, boxShadow: '0 8px 24px rgba(0,0,0,0.4)', minWidth: 180, padding: '4px 0' }}>
          ...
          <div style={{ padding: '12px 20px', borderTop: '1px solid #22283a', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 12, color: '#6b7280' }}>Mostrando {tenants.length === 0 ? 0 : (meta.page - 1) * meta.perPage + 1}-{Math.min(meta.page * meta.perPage, meta.total)} de {meta.total} clientes</span>
            <div style={{ display: 'flex', gap: 6 }}>
              <button disabled={page <= 1} onClick={() => setPage(p => p - 1)} style={{ padding: '6px 14px', fontSize: 12, fontWeight: 500, borderRadius: 6, border: '1px solid #22283a', cursor: page <= 1 ? 'not-allowed' : 'pointer', background: 'transparent', color: page <= 1 ? '#6b7280' : '#e8eaf0', opacity: page <= 1 ? 0.5 : 1 }}>Anterior</button>
              <button disabled={page >= meta.totalPages} onClick={() => setPage(p => p + 1)} style={{ padding: '6px 14px', fontSize: 12, fontWeight: 500, borderRadius: 6, border: '1px solid #22283a', cursor: page >= meta.totalPages ? 'not-allowed' : 'pointer', background: '#161a22', color: page >= meta.totalPages ? '#6b7280' : '#e8eaf0', opacity: page >= meta.totalPages ? 0.5 : 1 }}>Próximo</button>
            </div>
          </div>
        </div>
      )}
